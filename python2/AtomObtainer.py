...
def get_atoms_from_pw_output(pw_file_location, num_atoms=2):

    # open pw output file
    try:
        pw_out_file = open(pw_file_location, u'r')
    except IOError:
        print u'File read error.'
        return

    # find the cell type
    current_line, pw_out_file = move_to_line(pw_out_file, u'bravais-lattice index')
    cell_type = get_numbers_from_str(current_line, True)

    # read the lattice parameter
    current_line, pw_out_file = move_to_line(pw_out_file, u'celldm')
    # now we're on the correct line
    # split the string based on spaces
    split_line = current_line.split()
    # get celldm(1)
    cell_dm_1 = float(split_line[split_line.index(u"celldm(1)=")+1])/1.8897  # 1.8897 converts bohr units to Angstroms
    # do the others as and when needed

    # read the crystal axes a1, a2 and a3, put into basis
    current_line, pw_out_file = move_to_line(pw_out_file, u'crystal axes:')
    ...
    current_line, pw_out_file = move_to_line(pw_out_file, u'Begin final coordinates')
    current_line, pw_out_file = move_to_line(pw_out_file, u'ATOMIC_POSITIONS')
    ...
    current_line = pw_out_file.readline()
    muon = None
    while u'End final coordinates' not in current_line:
        # split the line up
...
def get_atoms_from_xtl(xtl_file_location):

    # open pw output file
    try:
        pw_out_file = open(xtl_file_location, u'r')
    except IOError:
        print u'File read error.'
        return
    ...
    current_line, pw_out_file = move_to_line(pw_out_file, u'CELL')
    current_line = pw_out_file.readline()
    # now we're on the correct line
    # split the string based on spaces
    split_line = current_line.split()
    ...
    current_line, pw_out_file = move_to_line(pw_out_file, u'ATOMS')
    current_line, pw_out_file = move_to_line(pw_out_file, u'NAME')
    ...
    current_line = pw_out_file.readline()
    muon = None
    while u'EOF' not in current_line:
        # split the line up
        atom_raw_data = current_line.split()
        atom_name = atom_raw_data[0]
        atom_position = coord.TCoord3D(float(atom_raw_data[1]), float(atom_raw_data[2]), float(atom_raw_data[3])
    ...
    muon_position = ask_atom_info(u'What is the position of the muon (in fractional coordinates)?', 3, isInt=False)
    muon_position = coord.TCoord3D(muon_position[0], muon_position[1], muon_position[2], prim_vectors)
    muon = atom(muon_position, gyromag_ratio=851.372, II=1, name=u'mu')
...
def move_to_line(file, line):
    ...
    while line not in current_line:
        try:
            ...
            print u'File format invalid'
            return
    return current_line, file


def get_numbers_from_str(string, IntOnly=False):
...
def get_atom_information(atom_name, atom_position, ask_if_muon=False, is_muon=False):
    print u"Found an atom, " + atom_name + u" at position " + unicode(atom_position) + u"\n"
    # ask if this is a muon
    if ask_if_muon:
        is_muon = query_yes_no(u"Is this a muon?", default=u"no")

    if is_muon:
        return False, atom(atom_position, gyromag_ratio=851.372, II=1, name=u'mu')

    # this is not a muon, so ask for the information
    atom_gyromag_ratio = ask_atom_info(u"Gyromagnetic ratio /MHz (*2pi)")
    if isinstance(atom_gyromag_ratio, np.ndarray):
        n_isotopes = len(atom_gyromag_ratio)
    else:
        n_isotopes = 1
    atom_II = ask_atom_info(u"2*I", n_isotopes, True)
    if n_isotopes > 1:
        atom_abundances = ask_atom_info(u"Abundances (decimal)", n_isotopes)
    else:
        atom_abundances = 1
...
def ask_atom_info(question, no_responses=None, isInt=False):
    while True:
        print question + u" (separate multiple with a space) > ",; sys.stdout.write(u"")
        response = raw_input()
        ...
        if no_responses is not None and no_responses != len(split_response):
            print u"Incorrect number of responses given"
        else:
