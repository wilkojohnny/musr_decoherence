...
import DecoherenceCalculator as DecoCalc  # decoherence hamiltonians etc
...
def main():
    ...
    calc_breit_rabi(muon_position=muon_position, squish_radius=squish_radii, lattice_type=lattice_type,
                    ...
                    fields=np.arange(0, 20, .1), field_polarisation=coord.TCoord3D(0,0,1),
                    outfile_location='/Users/johnny/Documents/University/CaF2/LCR/CaF2_zfield_nnnF_0_100.dat',
                    plot=True)
...
def calc_breit_rabi(muon_position, squish_radius=None, fields=np.arange(0, 1, 1e3),
    ...
    muon, All_Spins, got_atoms = AO.get_spins(muon_position, squish_radius, lattice_type, lattice_parameter,
                                           lattice_angles,
    ...
    if outfile_location is not None:
        ...
        breit_rabi_file_preamble(output_file, field_polarisation, fields, muon_position, All_Spins, use_xtl_input,
                                 xtl_input_location, use_pw_output, pw_output_file_location, perturbed_distances,
                                 squish_radius, nnnness, exclusive_nnnness, lattice_type, lattice_parameter)
    ...
    dipolar_hamiltonian = DecoCalc.calc_dipolar_hamiltonian(All_Spins)
    ...
    for i_field in range(0, len(fields)):
        ...
        hamiltonian = dipolar_hamiltonian + DecoCalc.calc_zeeman_hamiltonian(All_Spins, field_v)

        # print out the current field as a status update
        print('Field: ' + str(fields[i_field]) + 'G')

        # diagonalise the Hamiltonian
        E, _ = np.linalg.eigh(hamiltonian.todense())
    ...
    if plot:
        ...
        pyplot.ylabel('E /MHz')
        pyplot.title('F-mu-F, Field in z-direction')
        pyplot.show()
    ...
    # just for fun...
    ham_before = dipolar_hamiltonian + DecoCalc.calc_zeeman_hamiltonian(All_Spins, field_polarisation*12e-4)
    ham_after = dipolar_hamiltonian + DecoCalc.calc_zeeman_hamiltonian(All_Spins, field_polarisation*14e-4)
    Ebefore, Rbefore = np.linalg.eigh(ham_before.todense())
    Eafter, Rafter = np.linalg.eigh(ham_after.todense())

    R_diff = (Rbefore - Rafter).transpose()
    print(R_diff)
    pass


def breit_rabi_file_preamble(file, field_polarisation, fields, muon_position, nn_atoms, use_xtl_input=None,
                             xtl_input_location=None, use_pw_output=None, pw_output_location=None,
